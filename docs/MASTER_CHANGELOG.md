...
- Limpieza final de mojibake en `src/app/flights/[reservationId]/page.tsx` y `src/app/profile/audit/page.tsx` sin tocar l�gica ni payloads.
- Ajuste de textos owner en resumen de vuelo para presentaci�n p�blica (`test` -> `preview` en etiquetas visibles).
- QA visual por pantalla completado a nivel de estructura/c�digo: sin bloqueos cr�ticos detectados.
- Validaciones: `npm run build` OK y `npx tsc --noEmit` OK.
- Gate de release final queda condicionado al flujo de publicaci�n de versi�n/installer en bloque separado.

## 2026-05-10 - BLOQUE FINAL GLASS GLOBAL + CONTRASTE + QA
- Ajuste puntual del wrapper principal del dashboard a estilo glass claro (transl�cido + blur + borde blanco sutil + sombra premium), conservando estructura y flujo.
- Revisi�n final de contraste en dashboard para evitar bloque oscuro pesado sin tocar l�gica ni datos.
- Validaciones t�cnicas ejecutadas: `npm run build` OK y `npx tsc --noEmit` OK.
- Gate release condicionado a este bloque: sin cambios en APIs/Supabase/econom�a/scoring/SimBrief/Route Finder.
