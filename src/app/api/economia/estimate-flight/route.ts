  ...
  try {
    ...
    if (origin.length !== 4 || destination.length !== 4 || origin === destination) {
      return NextResponse.json(
        { ok: false, error: "Debes indicar origen y destino ICAO válidos y diferentes." },
        { status: 400 },
      );
    }
    ...
    const distanceNm =
      originAirport && destinationAirport
        ? haversineNm(originAirport, destinationAirport)
        : null;

    if (!distanceNm || distanceNm <= 0) {
      return NextResponse.json(
        { ok: false, error: "No se pudo calcular distancia para esos aeropuertos." },
        { status: 404 },
      );
    }

    const estimate = estimateRouteProfitLoss(distanceNm, aircraftType, operationType);
    ...
    return NextResponse.json({
      ...
      distanceNm,
      ...estimate,
      pilotPaymentUsd: estimate.pilotCommissionUsd,
    });
  } catch (error) {
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Error al estimar economía." },
      { status: 500 },
    );
  }
}
